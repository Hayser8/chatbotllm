...
import { getRobotsAgent } from "@/lib/utils/robots";
import { discoverSitemaps, collectSitemapUrls } from "@/lib/utils/sitemap";
...
export async function crawlSite(input: CrawlInput): Promise<CrawlOutput> {
  ...
  async function visit(node: { url: string; depth: number }) {
    ...
    // Siempre parseamos HTML para registrar edges; solo encolamos si hay profundidad disponible
  ...
  // -------------------------------
  // >>> POST-PASS: completar status de URLs del sitemap que TIENEN enlaces entrantes
  // pero quedaron sin fetch por límite de profundidad.
  // -------------------------------
  ...
  const candidates = [...fromSitemaps].filter((k) => {
    ...
    return inbound.has(k) && it && it.status === 0; // listadas en sitemap, con inbound y sin fetch
  });
    ...
    toFetch.map(async (k) => {
      try {
        ...
        inventoryMap.set(k, {
          ...
          discoveredBy: "both", // inferido: aparece en sitemap y recibe enlaces HTML
          redirectChain,
        });
        fetchedCount++;
      } catch {
  ...
  // Huérfanos REALES = en sitemap pero sin ningún enlace entrante
  const orphansInSitemap = [...sitemapSet].filter((k) => !inbound.has(k));
  // Enlazadas internamente pero no en sitemap
